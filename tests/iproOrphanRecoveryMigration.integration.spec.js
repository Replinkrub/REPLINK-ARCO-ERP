...
describeDisposable('IPRO migrations 020/021 orphan recovery audit', () => {
  it('records retired migration 021 without changing IPRO state, then skips it', async () => {
    await withDatabase(async (client) => {
      ...
      ) VALUES ('migration-021-retirement-state', 'PROCESSING', 'integration_test', 'migration-021-key', 'migration-021-fingerprint')`);
      const before = await retirementStateSnapshot(client);
      expect(before.appliedMigrations.find(({ filename }) => filename === migration020.filename))
        .toEqual({ filename: migration020.filename, checksum: calculateChecksum(migration020.sql) });

      expect(await runMigrations(client, [migration021], silentLogger())).toEqual({ applied: 1, skipped: 0 });
      expect((await client.query(
        "SELECT filename, checksum FROM schema_migrations WHERE filename = '021_ipro_runtime_role_binding_hardening.sql'"
      )).rows).toEqual([{ filename: migration021.filename, checksum: calculateChecksum(migration021.sql) }]);
      expect(await retirementStateSnapshot(client)).toEqual(before);
...
async function retirementStateSnapshot(client) {
  const tables = (await client.query(`SELECT table_name
    FROM information_schema.tables
    WHERE table_schema = 'ipro' AND table_type = 'BASE TABLE'
    ORDER BY table_name`)).rows.map(({ table_name }) => table_name);
  const tableRows = {};
  for (const table of tables) {
    const identifier = `"${table.replaceAll('"', '""')}"`;
    tableRows[table] = (await client.query(`SELECT row_to_json(state_row)::text AS row
      FROM (SELECT * FROM ipro.${identifier}) AS state_row
      ORDER BY row_to_json(state_row)::text`)).rows;
  }

  const appliedMigrations = await client.query(
    "SELECT filename, checksum FROM schema_migrations WHERE filename <> '021_ipro_runtime_role_binding_hardening.sql' ORDER BY filename"
  );
  const relationState = await client.query(`SELECT relation.relkind, relation.relname, pg_get_userbyid(relation.relowner) AS owner_name, relation.relacl::text AS acl
    FROM pg_class AS relation
    JOIN pg_namespace AS namespace ON namespace.oid = relation.relnamespace
    WHERE namespace.nspname = 'ipro'
    ORDER BY relation.relkind, relation.relname`);
  const routineState = await client.query(`SELECT routine.oid::regprocedure::text AS signature, pg_get_userbyid(routine.proowner) AS owner_name,
      routine.proacl::text AS acl, routine.proconfig
    FROM pg_proc AS routine
    JOIN pg_namespace AS namespace ON namespace.oid = routine.pronamespace
    WHERE namespace.nspname = 'ipro'
    ORDER BY signature`);
  const schemaState = await client.query(`SELECT pg_get_userbyid(nspowner) AS owner_name, nspacl::text AS acl
    FROM pg_namespace WHERE nspname = 'ipro'`);
  const roleState = await client.query(`SELECT rolname, rolsuper, rolinherit, rolcreaterole, rolcreatedb, rolcanlogin, rolreplication, rolbypassrls
    FROM pg_roles ORDER BY rolname`);
  const membershipState = await client.query(`SELECT parent.rolname AS parent_role, member.rolname AS member_role, membership.admin_option
  ...
  return {
    ...
    tableRows,
    relationState: relationState.rows,
    routineState: routineState.rows,
    schemaState: schemaState.rows,
    roleState: roleState.rows,
    membershipState: membershipState.rows,
  };
}
