...
describeDisposable('IPRO migration 020 orphan recovery audit', () => {
  it('applies to the clean 016-019 schema without creating IPRO runtime relations or pgcrypto', async () => {
    await withDatabase(async (client) => {
      ...
      expect(await runMigrations(client, [migration020], silentLogger())).toEqual({ applied: 0, skipped: 1 });
    });
  }, 60_000);
...
async function prepareBase(client) {
  ...
  return { base, migration020: migrations.find((migration) => migration.filename === '020_ipro_orphan_recovery_audit.sql') };
}
